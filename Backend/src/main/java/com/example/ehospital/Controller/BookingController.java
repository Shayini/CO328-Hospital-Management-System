...
public class BookingController {
    ...
    @ResponseBody
    public String addBooking(@PathVariable("userid") long userid, @PathVariable("doctorid") long doctorid,
                             @PathVariable("scheduleid") long scheduleid, @Valid @RequestBody Booking booking) {
        return user_repository.findByUserID(userid).map(user -> {
            booking_repository.insertBooking(booking.getDepartment(), doctorid, scheduleid, booking.getDate(),
                    booking.getTime(), userid, booking.getContact_no(), booking.getEmail());
