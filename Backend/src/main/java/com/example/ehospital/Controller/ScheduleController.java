...
public class ScheduleController {
    ...
    @Transactional
    @PostMapping("/doctor/{doctorid}/addschedule")
    @ResponseBody
    public String addSchedule(@PathVariable("doctorid") long doctorid, @Valid @RequestBody Schedule schedule) {
        return doctorRepository.findByDoctorID(doctorid).map(doctor -> {
            if(scheduleRepository.isScheduleExistByDoctorIDAndDate(doctorid, schedule.getDate()) == 1) {
                return "Schedule Already Added";
            }
            scheduleRepository.insertSchedule(schedule.getFrom_time(), schedule.getTo_time(),
                    schedule.getDate(), doctorid);
            return "Schedule Added Successful";
...
//    }
    ...
    public String deleteSchedule(@PathVariable (value = "id") long doctorid,
